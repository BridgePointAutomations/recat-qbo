...
describe('Reports', () => {
  it('keeps P&L filters usable after failure and retries current filters', async () => {
    ...
    await user.selectOptions(screen.getByRole('combobox', { name: 'Accounting method' }), screen.getByRole('option', { name: 'Accrual' }));
    expect(await screen.findByRole('alert')).toHaveTextContent('Profit & Loss could not load');
  ...
  it('shows Balance Sheet failure in place and retries', async () => {
    ...
    render(<Reports />);
    await user.selectOptions(screen.getByRole('combobox', { name: 'Report' }), screen.getByRole('option', { name: 'Balance Sheet' }));
  ...
  it('announces a pending transaction-log read', async () => {
    ...
    render(<Reports />);
    await user.selectOptions(screen.getByRole('combobox', { name: 'Report' }), screen.getByRole('option', { name: 'Transaction log' }));
...
it('clears the previous transaction log while a custom range is incomplete', async () => {
  const user = userEvent.setup();
  render(<Reports />);
  await user.selectOptions(screen.getByRole('combobox', { name: 'Report' }), 'txns');
  expect(await screen.findByText('No transactions in this period.')).toBeVisible();
...
it('ignores an older statement response after the filters change', async () => {
  ...
  const user = userEvent.setup();
  render(<Reports />);
  await user.selectOptions(screen.getByRole('combobox', { name: 'Accounting method' }), 'accrual');
